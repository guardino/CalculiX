...
int includedNodes( int set1, int set2, double cof[4])
{
  int i,j,e,n;
  int in, etet, tets=0, extrapolflag=-1, includedFlag=0;
  ...
  /* check which nodes of setNr are inside the elements of elsetNr */
  delSet(specialset->penetration);
  in=pre_seta(specialset->penetration,"i",0);
  for(i=0; i<set[set1].anz_n; i++)
  {
    ...
    if(etet>-1)
    {
      includedFlag++;
      printf(" n:%d\n",set[set1].node[i]);
      seta(in,"n",set[set1].node[i]);
      if(printFlag) printf(" node:%d coef:%e %e %e %e\n",set[set1].node[i], cof[0], cof[1], cof[2], cof[3]);
      /*
        sprintf(parameter[0],"%d",set[set1].node[i]);
        sprintf(parameter[1],"%e",cof[0]);
        sprintf(parameter[2],"%e",cof[1]);
        sprintf(parameter[3],"%e",cof[2]);
        sprintf(parameter[4],"%e",cof[3]);
...
void separateSurfs(int setSplit, int setSurf)
{
  ...
  if(set[setbuf].anz_nurs>0)
  {
    ...
    for (s=0; s<b; s++)
    {
      ...
      nurbs[nr].nurbsType=nurbs[S].nurbsType;
      nurbs[nr].axis=nurbs[S].axis;
...
int prnt(char *record)
{
  ...
  if ((compare( typ, "capt", 3) == 3) ||(compare( typ, "CAPT", 3) == 3))
  {
    printf("%s\n", picture_caption);
    strcpy(parameter[0], picture_caption);
    write2stack(1, parameter);
    return(1);
  }
  if ((compare( typ, "ulin", 3) == 3) ||(compare( typ, "ULIN", 3) == 3))
  {
    printf("%s\n", picture_text);
    strcpy(parameter[0], picture_text);
    write2stack(1, parameter);
    return(1);
  }
  ...
  else if ((compare( typ, "se", 2) == 2) ||(compare( typ, "SE", 2) == 2))
  {
    ...
    for (i=0; i<set[setNr].anz_p; i++)
    {
      ...
      {
        printf (" pnt %s %le %le %le\n", point[p].name,
        (point[p].px* scale->w+scale->x),
	(point[p].py* scale->w+scale->y),
	(point[p].pz* scale->w+scale->z) );
        sprintf(parameter[0],"%s", point[p].name);
        sprintf(parameter[1],"%le", point[p].px* scale->w+scale->x);
        sprintf(parameter[2],"%le", point[p].py* scale->w+scale->y);
        sprintf(parameter[3],"%le", point[p].pz* scale->w+scale->z);
        write2stack(4, parameter);
      }
    }
    for (i=0; i<set[setNr].anz_l; i++)
...
int zap(char *record)
{
  ...
  if (set[setNr].anz_b>0)
  {
	...
	if (gbod_i( body[b].name, 0, s, cori, edge )<=0) { bodybuf[anz_b]=b; anz_b++; }
	else
	{
  ...
  */
  ...
  else if (compareStrings(type, "SETR")>0)
  {
	...
	nr=0;
        ...
        if(nr>0) { setr( setNr, type, nr); goto leaveSetr; }
	
	nr=-1;
        if (type[0]=='v') nr=getValuNr(dat[i]);
        ...
        if(nr>-1) { setr( setNr, type, nr); goto leaveSetr; }
  ...
  else if (compareStrings(type, "TEST")>0)
  {
    ...
    if ((type[0]=='i')&&(type[1]=='\0'))
    {
      ...
      na = includedNodes(setNr,nr, &cof[0]);
      if(na>=1)
      {
    ...
    if ((type[0]=='o')&&(type[1]=='\0'))
    {
      ...
      strcpy(parameter[0],"FALSE");
      ibuf=atoi(name);
      if ((ibuf>=anz->nmin)&&(ibuf<=anz->nmax)&&(node[ibuf].indx>=0)&&(ibuf==node[node[ibuf].indx].nr)&&(node[ibuf].pflag==0))
      {
...
int pre_read( char *record )
{
  ...
  else if((compare( ext, "frd", 3)== 3)||(compare( param1, "ng", 2)== 2)||(compare( ext, "stl", 3)== 3)||(compare( ext, "inp", 3)== 3)||(compare( param1, "inp", 3)== 3)||(compare( param1, "foam", 4)== 4)||(compare( param1, "vtk", 3)== 3)) 
  {
    ...
    else
    {
      apre->nmax=anz->nmax;
      apre->emax=anz->emax;
      if( readccx( datin, apre, &spre, &npre, &epre, &lpre, 0) == -1) return(0);
    }
...
int pre_mesh( char *record )
{
  ...
  scipMesh:;
    ...
    printf("delete surface elements\n");
    if(etmp>0) zap( "+tetSet" );
    printf("surface elements deleted\n");
  }
...
int completeSet( char *setname, char *type )
{
  ...
  if (toupper(type[0])=='U') /* up  */
  {
    ...
    if (set[setNr].anz_n>0)
    {
      ...
      {
        ipuf=0;
        if (face[i].type == 7) ipuf = 3;  /* TRI3  */
        else if (face[i].type == 8) ipuf = 6;  /* TRI6  */
        else if (face[i].type == 9) ipuf = 4;  /* QUAD4 */
        else if (face[i].type == 10) ipuf = 8; /* QUAD8 */
        else if (face[i].type == 11) ipuf = 2; /* beam2 */
        else if (face[i].type == 12) ipuf = 3; /* beam3 */
        if (ipuf!=0)
        {
          for (j=0; j<ipuf; j++)
          {
  ...
  else if (toupper(type[0])=='F') /* faces */
  {
    ...
    /* go over all faces and look if all their nodes are included in the set setNr */
    m=set[setNr].anz_f;
    for (i=0; i<anz->f; i++)
    { 
  ...
  else if (toupper(type[0])=='E') /* elements */
  {
    if ( setNr==getSetNr(specialset->mesh)||( set[setNr].type==1)||(setNr==getSetNr(specialset->nomesh))) return(-1);
    
    /* add the faces to the sets which fit fully to the included nodes */
    if(printFlag) printf(" include elements in sets\n");

    /* go over all elements and look if all their nodes are included in the set setNr */
    m=set[setNr].anz_e;
    for (i=0; i<anz->e; i++)
    { 
      ipuf=0;
      ...
      for (j=0; j<ipuf; j++)
      {
	if(ifind(&set[setNr].node, set[setNr].anz_n, e_enqire[e_enqire[i].nr].nod[j])>-1) k++;
      }
      if(k==ipuf)
      {
        if ( (set[setNr].elem = (int *)realloc((int *)set[setNr].elem, (set[setNr].anz_e+1) * sizeof(int))) == NULL )
    ...
    /* go over all elements and look if all of their faces are included in the set setNr */
    // TBD
    //m=set[setNr].anz_e;
    //for (i=0; i<anz->e; i++)
  }
  ...
  else if (toupper(type[0])=='P') /* edges */
  {
    /* cyrcle through all bodys and add all surfs */
    for (i=0; i<set[setNr].anz_b; i++)
    {
...
#define DT_KELVIN_TO_C -273.15
  ...
  for (i=0; i<set[setNr].anz_n; i++)
  {
    ...
    node2[isort[i]].pflag = node[isort[i]].pflag;
    node2[isort[i]].indx = anz2->n;
  ...
  else if (compare( format, "frd", 3) == 3)
  {
    ...
    if((val)&&((compare( val[0], "ds", 2)== 2)||(compare( val[0], "bin", 2)== 2)||(compare( val[0], "dbin", 3)== 3)))
    {
      ...
      if(lc)
      {
        ...
        lcase2[0].loaded=lcase[lc].loaded;
