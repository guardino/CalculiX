...
void delElem( int anze, int *index ) 
{
  ...
  for (j=0; j<anze; j++)
  {
    ...
    if(e_enqire[buf[j]].type>10)
    {
      for (k=0; k<anzGeo->l; k++)
      {
        if(line[k].ne<1) continue;
	if(buf[j]<line[k].elem[0]) continue;
	if(buf[j]>line[k].elem[line[k].ne-1]) continue;
        for (n=0; n<line[k].ne; n++)
        {
          if(line[k].elem[n]==buf[j]) { line[k].elem[n]=0; break; }
        }
      }
    }
    else if(e_enqire[buf[j]].type>6)
    {
...
void operateAlias( char *name, char *type )
{
  ...
  else if( name[0] == '?')
  {
	...
	{
          printf("alias name:%s entity type:%s\n",alias[i].name,alias[i].type);
	  length=strlen(name);
	  for(i=0; i<length; i++) name[i]=name[i+1];
          //strcpy( name, &name[1] );
          return;
  ...
  else if( name[0] == '!')
  {
    ...
    i=strlen(type);
    if((alias[alnam].type= (char *)malloc((i+1)*sizeof(char))) == NULL )
    { printf("ERROR: malloc failed\n\n" ); return; }
    strcpy( alias[alnam].type, type );
    if(printFlag) printf("  real name:%s type:%s\n", alias[alnam].entityName, alias[alnam].type);
  }
}
...
int pre_value( char *record)
{
  ...
  else if(compareStrings(string, "pop")>0) 
  {
	...
	j=strlen(string)-1; while((j>=0)&&(string[j]==' ')) { string[j]=0; j--; }
...
int  gbod_i( char *name, int blend, int anz_c, char *cori, int *edge )
{
  ...
  */
  ...
  if ( i==0 )
  {
      errMsg ("WARNING: orientBody:%s failed for an unstructured body since no element type was specified\n", name);
      //body[nr].name = (char *)NULL ;
      return(0);
  }
  for (i=0; i<anz->sets; i++)
